...
function dev_theme_scripts() {
	wp_enqueue_style( 'dev-theme-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'dev-theme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'dev-theme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
/* Remove theme support for image zoom - only works in twentytentytwo */
...
// function display_acf_field_on_product_page() {

//     $test_value = get_field('test');
// 	$hover_image = get_field('hover_image');

//     if ($test_value) {
//         echo '<div class="acf-test-field">' . esc_html($test_value) . '</div>';
//     }

// 	if ($hover_image) {
// 		echo '<img src="'. $hover_image['url'] .'" alt="my image">';
// 	}
// }

// add_action('woocommerce_before_single_product', 'display_acf_field_on_product_page');
